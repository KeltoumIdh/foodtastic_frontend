...
const predefinedCities = [
  ...
  },
  {
    name: "Casablanca",
    imageUrl:
      "https://www.leconomiste.com/sites/default/files/eco7/public/casablanca_budget_trt_bon.jpg",
  },
  {
    name: "Montreal",
    imageUrl:
      "https://www.montrealinternational.com/app/uploads/2019/03/montreal.jpg",
  },
  {
    name: "Berlin",
    imageUrl:
...
const Landing = () => {
  ...
  useEffect(() => {
    const fetchProductsByCity = async () => {
      try {
        const response = await axios.get(`/api/productsByCity`, {
          params: { city },
        });
    ...
    const fetchCities = async () => {
      try {
        ...
        const mergedCities = fetchedCities.map((city) => {
          const predefinedCity = predefinedCities.find(
            (c) => c.name === city.name
          );
        ...
        const uniqueCities = [
          ...predefinedCities,
          ...mergedCities.filter(
            (city) => !predefinedCities.some((c) => c.name === city.name)
          ),
        ];
    ...
    <main>
      ...
      {!loginState ? (
        <div className="selected-products">
          ...
          <div
            className={`${
              cities.length > 5 ? "lg:grid-cols-3" : "lg:grid-cols-5"
            } grid grid-cols-3 gap-10 max-w-7xl mx-auto`}
          >
            {cities.map((city) => (
              <Link to={`/shop?city=${city.name}`} key={city.name}>
                <article
                  key={city.name}
