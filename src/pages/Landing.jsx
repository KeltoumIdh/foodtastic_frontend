...
const Landing = () => {
  ...
  const [cities, setCities] = React.useState(predefinedCities);
  const randomPhotos = [
    ...
    const fetchCities = async () => {
      try {
        ...
        const mergedCities = fetchedCities.map(city => {
          ...
          return predefinedCity ? predefinedCity : {
            name: city.name,
        ...
        const uniqueCities = [...predefinedCities, ...mergedCities.filter(city => !predefinedCities.some(c => c.name === city.name))];
      ...
      {!loginState ? (
        <div className="selected-products">
          ...
          <div className="grid grid-cols-5 gap-10 max-w-7xl mx-auto">
            {cities.map((city) => (
              <article key={city.name} className="relative isolate flex flex-col cursor-pointer justify-end w-full overflow-hidden rounded-2xl px-8 pb-8 pt-40 max-w-sm mx-auto">
                <img
                  src={city.imageUrl}
                  alt={city.name}
                ...
                <h3 className="z-10 mt-3 text-2xl font-bold text-white">{city.name}</h3>
              </article>
            ))}
          </div>
        </div>
